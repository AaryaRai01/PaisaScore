...
export default function OfficerReportsPage() {
  ...
  return (
    <div className="p-8 space-y-8">
      <header className="flex justify-between items-start border-b pb-8 print:border-b-2 print:border-[var(--color-primary)] print:mb-10">
        <div className="flex flex-col gap-2">
          <div className="hidden print:block mb-6">
            <div className="flex items-center gap-4">
              <img src="/officer and user dashboard logo.png" alt="PaisaScore" className="h-14 w-auto" />
              <div className="h-10 w-[2px] bg-slate-200" />
              <div>
                <p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400">PaisaScore Financial Architect</p>
                <p className="text-xs text-slate-500 font-medium">Enterprise Portfolio Intelligence</p>
              </div>
            </div>
          </div>
          <h2 className="font-headline text-4xl font-extrabold text-slate-900 tracking-tight">Portfolio Performance Report</h2>
          <p className="text-slate-500 mt-1">Live data analytics and credit risk distribution overview.</p>
        ...
        <div className="flex gap-3 no-print">
          <button onClick={handleCSV} className="flex items-center gap-2 px-5 py-2.5 bg-slate-100 text-slate-700 rounded-xl font-semibold text-sm hover:bg-slate-200 transition-colors">
            <span className="material-symbols-outlined text-lg">grid_on</span>Export for Sheets
          </button>
          <button onClick={() => { showToast("Preparing professional report...", "info"); setTimeout(() => window.print(), 500); }}
            className="flex items-center gap-2 px-5 py-2.5 bg-[var(--color-primary)] text-white rounded-xl font-semibold text-sm hover:opacity-90 transition-opacity">
            <span className="material-symbols-outlined text-lg">description</span>Generate PDF Report
          </button>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8 bg-white rounded-xl shadow-sm border border-slate-100 p-6">
          ...
          {chartData.length === 0 ? (
            ...
            <div className="flex items-end gap-6 h-48 border-b border-l border-slate-100 pl-2 pb-2">
              {chartData.map((d) => {
                const maxH = 180;
                ...
                return (
                  <div key={d.type} className="flex-1 flex flex-col items-center gap-2 group relative">
